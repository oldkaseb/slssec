...
from telegram import (
    Update, InlineKeyboardButton, InlineKeyboardMarkup, ChatMemberUpdated, ChatPermissions,
    InputMediaPhoto, InputMediaVideo, InputMediaAudio, InputMediaDocument, MessageEntity
)
...
async def handle_pm_album(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kind = st["kind"]
    try:
        header = f"📨 آلبوم از {mention(user.id, user.full_name)}\n@{user.username or '-'} | id: `{user.id}`"
        await context.bot.send_message(
            chat_id=GUARD_CHAT_ID if kind=="guard" else OWNER_ID,
            text=header,
            parse_mode=ParseMode.MARKDOWN
        ...
        logger.exception("album forwarding failed: %s", e)
...
async def cmd_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in reversed(rows):
        d = r["date"]
        if jdatetime: jd = jdatetime.date.fromgregorian(date=d).strftime("%Y/%m/%d")
        else: jd = d.strftime("%Y-%m-%d")
        lines.append(f"• {jd}: پیام {r['messages_count']} | رسانه {r['media_count']} | ویس {r['voice_count']} | منشن {r['mentions_made_count']} | کال {format_secs(int(r['call_time_sec']))}")
    cap = "\n".join(lines)
    if file_id:
